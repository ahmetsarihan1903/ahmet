      ...
      <div className="bg-slate-900 rounded-lg p-5 sm:p-6 shadow-2xl border border-slate-800">
        ...
        <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-800">
          <button
            type="button"
            onClick={onBack}
            className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-300 hover:text-white px-3 py-1.5 rounded bg-slate-800 hover:bg-slate-700 border border-slate-700 transition-colors cursor-pointer"
          >
        ...
        <div className="space-y-4">
          ...
          <div className="bg-slate-950 p-3.5 rounded border border-slate-800">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              ...
              >
                ...
                <span className="text-[10px] opacity-80">Makine Daireli</span>
              </button>
              ...
              >
                ...
                <span className="text-[10px] opacity-80">Makine Dairesiz</span>
              </button>
            </div>
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-1.5 flex-wrap pt-1">
              <span className="text-[10px] text-slate-400 uppercase font-mono">Hızlı Seçim:</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 bg-slate-950 p-3.5 rounded border border-slate-800">
            {/* 4. Durak Sayısı */}
            <div className="space-y-1">
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none bg-slate-900 border border-slate-700 rounded p-2.5 text-xs sm:text-sm font-bold text-slate-100 focus:outline-none focus:border-blue-500 cursor-pointer"
                >
                  {stopOptions.map((num) => (
            ...
            <div className="sm:col-span-2 pt-2 border-t border-slate-800">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5 text-[11px] text-slate-300">
                  ...
                  <span>Kat Matrisi: 1.Durak = <strong className="text-blue-300">{floorMatrix[0]?.floorLabel}</strong> ... {stopCount}.Durak = <strong className="text-blue-300">{floorMatrix[floorMatrix.length - 1]?.floorLabel}</strong></span>
                </div>
                <button
                  type="button"
              ...
              {showMatrixPreview && (
                <div className="mt-2 max-h-40 overflow-y-auto rounded border border-slate-800 bg-slate-900 p-2 text-xs">
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                    {floorMatrix.map((item) => (
                      <div key={item.stopIndex} className="p-1.5 bg-slate-950 rounded border border-slate-800 flex justify-between items-center">
                        <span className="text-slate-400 font-mono">{item.stopIndex}. Durak:</span>
                        <span className="text-blue-300 font-bold">{item.floorLabel}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="bg-slate-950 p-3.5 rounded border border-amber-500/30 space-y-2.5">
          ...
          <div className="pt-2">
            <button
              ...
              className={`w-full py-3.5 px-4 rounded font-black text-sm uppercase tracking-wider flex items-center justify-center gap-2 transition-all shadow-lg ${
                isUnlocked
                  ? 'bg-orange-500 hover:bg-orange-400 text-white shadow-orange-500/30 active:scale-[0.99] cursor-pointer'
            ...
            {!isUnlocked && (
              <div className="mt-3 p-2.5 bg-slate-950 rounded border border-slate-800 text-[11px] text-slate-400 space-y-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
                  <span className={isSerialValid ? 'text-emerald-400' : 'text-amber-400/80'}>
                    {isSerialValid ? '✓' : '•'} Seri Numarası girilmeli
                  </span>
                  <span className={isCapacityValid ? 'text-emerald-400' : 'text-amber-400/80'}>
                    {isCapacityValid ? '✓' : '•'} Kapasite (KG) girilmeli
                  </span>
                  <span className={isDateTimeConfirmed ? 'text-emerald-400' : 'text-amber-400/80'}>
                    {isDateTimeConfirmed ? '✓' : '•'} 6. Madde Onayı (Tarih/Saat)
                  </span>
                  <span className={isHardwareMatched ? 'text-emerald-400' : 'text-amber-400/80'}>
