...
export const Step3SwipePanel: React.FC<Step3SwipePanelProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-12 sm:top-14 z-30 bg-white border-b-2 border-slate-300 -mx-3 sm:-mx-4 px-3 sm:px-4 mb-3 shadow-md">
        {/* Navigation Action Buttons: Önceki | DENETİMİ BİTİR | Sonraki */}
        <div className="py-2 flex items-center justify-between gap-2 border-b border-slate-200">
          {/* Önceki Button */}
          <button
            type="button"
            id="btn-top-prev-tab"
            ...
            className={`px-3 py-2 rounded font-bold text-xs uppercase tracking-wider flex items-center gap-1 transition-all shrink-0 select-none ${
              activeTab === 0
          ...
          <button
            ...
            className="flex-1 max-w-md py-2 px-2.5 sm:px-4 rounded font-bold text-xs sm:text-sm uppercase tracking-wider flex items-center justify-center gap-1.5 sm:gap-2 bg-red-600 hover:bg-red-700 active:scale-[0.99] text-white shadow transition-all cursor-pointer select-none"
          ...
          <button
            ...
            className={`px-3 py-2 rounded font-bold text-xs uppercase tracking-wider flex items-center gap-1 transition-all shrink-0 select-none ${
              activeTab === tabs.length - 1
                ? 'bg-slate-100 text-slate-400 cursor-not-allowed border border-slate-200'
                : 'bg-orange-500 hover:bg-orange-600 active:scale-95 text-white shadow cursor-pointer'
            }`}
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-2 sm:py-2.5 border-b-4 text-[11px] font-bold uppercase tracking-tight shrink-0 transition-all cursor-pointer ${
                  isActive
                    ? 'border-blue-600 text-blue-700 bg-blue-50/80 font-black'
                    : 'border-transparent text-slate-600 hover:text-slate-900 opacity-75 hover:opacity-100'
                }`}
              >
                ...
                {tab.id === 0 ? (
                  count > 0 && (
                    <span className="ml-1 px-1.5 py-0.2 rounded text-[10px] font-bold bg-blue-100 text-blue-800 border border-blue-200">
                      {count}
                    </span>
                  )
                ) : count > 0 ? (
                  <span className="ml-1 px-1.5 py-0.2 rounded text-[10px] font-black bg-red-600 text-white animate-pulse">
                    {count} UD
                  </span>
                ) : null}
              </button>
      ...
      <div className="bg-white rounded p-3 sm:p-3.5 shadow-sm border border-slate-300 mb-3 flex items-center justify-between">
        ...
        {activeTab === 2 && (
          <div className="space-y-3">
            <div className="bg-slate-100 p-3 rounded-xl border border-slate-200 text-xs text-slate-700 flex items-center justify-between">
              <span className="font-bold">Asansör Yapısı: {elevatorType === 'MR' ? 'Makine Daireli (MR)' : 'Makine Dairesiz (MRL)'}</span>
              <span className="text-slate-500 font-medium">{motorChassisItems.length} Kontrol Maddesi</span>
            </div>
        ...
        {activeTab === 7 && (
          <div className="space-y-3">
            <div className="bg-slate-100 p-3 rounded-xl border border-slate-200 text-xs text-slate-700">
              <span className="font-bold block">Otomatik Oluşturulan Kat Kapısı Matrisi</span>
        ...
        {currentTabInfo.allowExtra && activeTab !== 0 && (
          <div className="pt-2">
            <button
              ...
              className="w-full py-2.5 px-3 rounded border border-dashed border-slate-300 hover:border-slate-400 bg-white hover:bg-slate-50 text-slate-700 font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all cursor-pointer"
            >
